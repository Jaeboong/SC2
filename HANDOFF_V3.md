...
- 야생 저그 P15는 정상 Computer 슬롯과 소유권·동맹을 유지한다. `MeleeInitAI()` 직후
  main state를 Disabled, attack state를 Wait로 두고 420 게임초의 일회성 트리거에서
  Init으로 전환한다. 별도 harvest/전투/생산 명령은 발주하지 않는다.
...
야생 저그 지연도 별도 엔진 실행으로 확인했다. P15 일꾼은 5초부터 419초까지
20기로 유지됐고 420초 해제 후 480초에는 25기로 증가했다. 1~12번과 P15 소유자는
전 구간 보존되어 플레이어 슬롯 소실도 재현되지 않았다.
