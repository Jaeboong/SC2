...
V3는 Blizzard 설치 파일을 수정하지 않는다. 고정 upstream 54개와 V3 overlay 10개를
...
- 후속 확장은 분 단위 목표를 사용하지 않는다. 이전 확장이 `Established`가 되고
  건설 중인 town이 없을 때 하나씩 진행하며, 4개 채광 기지까지는 우선 확보한다.
  4기지 이후에는 전체 광물 일꾼이 포화된 경우에만 최대 6기지까지 확장한다.
- 모든 고정 일꾼 Stock은 완성된 채광 town의 `광물 지점×2 + 완성 가스×3`으로
  제한한다. 아직 완성되지 않은 미래 확장의 일꾼을 미리 생산하지 않는다.
- 완성된 본진 외 채광 town은 raw gas spot 수만큼 town-local 가스 건물을 요청한다.
  완성되어 harvestable 상태가 된 가스마다 일꾼 3명을 배정한다.
...
V3.3 단독 18분 기록은 `v3/docs/verification/v3.3-engine-log.md`에 있다. 여섯 빌드의
일꾼·가스·업그레이드·병력은 실제로 진행했고, 저그는 울트라까지 생산했다. 이 작은
